...
#内存
 ...
 used_memory_human)
        result=`$REDIS_PA|/bin/grep used_memory_human|awk -F":" '{print $NF}'|awk -F'K' '{print $1}'` 
            ...
            ;; 
 used_memory_rss)
        result=`$REDIS_PA|/bin/grep used_memory_rss|grep -v used_memory_rss_human|awk -F":" '{print $NF}'`
            echo $result 
            ;; 
 used_memory_peak)
 ...
 used_memory_peak_human)
        result=`$REDIS_PA|/bin/grep  used_memory_peak_human|awk -F":" '{print $NF}'|awk '{if ($1~"M") {printf("%.2f\n",$1*1024)} else {print$1}}'|awk -F'K' '{print $1}'`
            echo $result 
            ;; 
 used_memory_lua)
        result=`$REDIS_PA|/bin/grep used_memory_lua|grep -v used_memory_lua_human|awk -F":" '{print $NF}'`
            echo $result 
            ;;     
